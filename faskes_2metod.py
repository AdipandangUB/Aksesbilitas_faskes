...
# ============================================================
# FUNGSI UTILITAS UNTUK CONCAVE HULL (TANPA ALPHASHAPE)
# ============================================================
def calculate_concave_hull_alternative(points, ratio=0.1):
    """
    Alternatif untuk alphashape menggunakan shapely.concave_hull
    """
    try:
        if len(points) < 3:
            return None
        ...
        concave_polygon = concave_hull(multipoint, ratio=ratio)
        
        if concave_polygon and not concave_polygon.is_empty:
            return concave_polygon
        else:
            return convex_hull(multipoint)
            
    except Exception as e:
        st.warning(f"Error dalam concave hull alternatif: {e}")
        return None

def calculate_alpha_shape(points, alpha=None):
    """
    Menghitung alpha shape (concave hull) dari sekumpulan titik
    MENGGUNAKAN SHAPELY CONCAVE_HULL sebagai alternatif
    """
    try:
        if len(points) < 3:
            return None
...
def optimize_alpha_value(points, target_coverage=0.95):
    """
    Mengoptimalkan nilai ratio untuk concave hull
    """
    try:
        if len(points) < 3:
            return None, 0.1
        
        best_shape = calculate_alpha_shape(points)
        best_ratio = 0.1
        
        return best_shape, best_ratio
    ...
    try:
        ...
        if len(unique_points) >= 3:
            try:
                ...
                if concave_shape and not concave_shape.is_empty:
                    ...
                    st.info(f"✅ Menggunakan Concave Hull")
                else:
        ...
        if service_area is None or service_area.is_empty:
            try:
                ...
                service_area = start_point.buffer(max_distance * 0.8)
...
def analyze_from_point_main(location_point, network_type, speed_kmh, radius_m, time_limits_min, 
    ...
    DIPERBAIKI: Mencegah re-rendering yang tidak perlu
    """
    ...
    try:
        ...
        result = (graph_proj, health_facilities, accessibility_zones, reachable_edges_dict)
...
# ============================================================
def display_results(location_point, graph, health_facilities, accessibility_zones, 
                   reachable_edges, area_calculation_method, travel_speed, 
                   search_radius, time_limits, **kwargs):
    """
    Fungsi terpisah untuk menampilkan hasil analisis
    """
    ...
    with tab2:
        ...
        with col4:
            st.metric(
                "🚗 Mode Transportasi",
                kwargs.get('mode_bahasa', 'mobil/motor').capitalize(),
                delta="Terpilih"
            )
        with col5:
            st.metric(
        ...
        if area_calculation_method == "Service Area":
            if 'hull_method' in kwargs and "Concave Hull" in kwargs['hull_method']:
                method_info = f"""
                ...
                **Algoritma yang digunakan:**
                1. **Dijkstra Algorithm**: Menghitung jarak terpendek dari titik awal
                2. **Identifikasi Edges Terjangkau**: Edges yang terjangkau berdasarkan jarak
                3. **Buffer Each Edge**: Setiap edge dibuffer berdasarkan jaraknya
            ...
            **Algoritma yang digunakan:**
            ...
            2. **Shape Selection**: Bentuk buffer ({kwargs.get('buffer_shape', 'Lingkaran')})
...
with st.sidebar:
    ...
    analyze_button = st.button("🚀 Jalankan Analisis", type="primary", use_container_width=True, key="analyze_button")
        ...
        if st.session_state.analysis_in_progress:
            ...
            if st.session_state.analysis_params != current_params:
                ...
                if result[0] is not None or area_calculation_method == "Buffer dari Titik":
                    ...
                    display_results(location_point, graph, health_facilities, 
...
st.markdown(
    ...
    <div style='text-align: center; padding: 20px;'>
        ...
        Developer: <b>Adipandang Yudono, S.Si., MURP., PhD <b>(Spatial Analysis, Architecture System, Scrypt Developer, WebGIS Analytics), <b>Dr (Cand). Firman Afrianto<b> (QGIS Plugin), <b>dr. Nuretha Hevy Purwaningtyas, M.Sc, Sp.DLP<b> (health behaviour analysis), <b>Dr. Diah Royani Meisani, S.S., M.Pd.<b> (Database), <b>dr. Aurick Yudha Nagara, Sp.EM, KPEC<b> (Health Facilities accessibilties), <b>dr. Fatah Abdul Yasir<b> (Health Facilities accessibilties) &  <b>dr. Rico Wicaksana Putra<b> (Health Facilities accessibilties).
        <br>
